...
From the beginning, I took one element that extremely important most specifically PM2.5 pollution (particulars matter 2.5 microns or less) while ignoring some of the others then they use only one ML model. First simple regression analysis. Each machine learning model is special and it has upsides and downsides. RNN accurate but required so much data. Random Forest robust but tend to overfit. Take a multilateral approach and use multiple machine learning methodology.
